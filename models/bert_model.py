...
def get_optimizer_grouped_parameters(
    model, learning_rate, weight_decay, layerwise_learning_rate_decay
):
    """
...
def get_llrd_optimizer_scheduler(
    model,
    learning_rate=1e-5,
    weight_decay=0.01,
    ...
    optimizer = AdamW(grouped_optimizer_params, lr=learning_rate, correct_bias=True)
...
def ohem_loss(preds, labels, weights, label_smoothing=0.05):
    """
    ...
    """
    CE_LOSS_OBJ = torch.nn.CrossEntropyLoss(
...
class MLMDataset(Dataset):
    ...
    def __init__(self, tokenizer, text_list: list, block_size: int = 128):
        batch_encoding = tokenizer(
            text_list, add_special_tokens=True, truncation=True, max_length=block_size
        )
...
class StratifiedBatchSampler:
    ...
    def __init__(self, y, batch_size, shuffle=True):
        ...
        self.skf = StratifiedKFold(
            n_splits=self.n_batches, shuffle=shuffle, random_state=1337
        )
...
class BertModel(BaseModel):
    ...
    def load(self):
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(
            self.model_path, use_auth_token=self.auth_token, num_labels=self.num_labels
        )
    ...
    def train_mlm(self, x_train):
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            self.model_path, ignore_mismatched_sizes=True, add_prefix_space=True
        )
    ...
    def train(self, x_train, y_train, x_val=0, y_val=0, n_batches=1, fold_id="none"):
        """
        ...
        cls_weights = list(
            dict(
                sorted(
                    dict(
                        1 / ((y_train.value_counts(normalize=True)) ** (1 / 3))
                    ).items()
                )
            ).values()
        )
        ...
        self.model.to(self.device)
        self.tokenizer = AutoTokenizer.from_pretrained(
            model_path, ignore_mismatched_sizes=True, add_prefix_space=True
        )
        ...
        if self.prevent_bias == 2:
            ...
            offensive_lens = (
                ...
                .str.split()
                .apply(len)
                .sample(len(x_train[y_train == 0]), replace=True)
                .values
            )

            k = (
                pd.DataFrame(x_train[y_train == 0].str.split())
                .reset_index(drop=True)
                .reset_index()
            )
            k = k.apply(
                lambda x: " ".join(x.text[: offensive_lens[x["index"]]]), axis=1
            ).values
        ...
        train_loader = DataLoader(
            ...
            batch_sampler=StratifiedBatchSampler(
                y_train, batch_size=self.batch_size, shuffle=True
            ),
            num_workers=0,
            pin_memory=False,
        )
        ...
        for epoch in range(self.epochs):
            ...
            optimizer.zero_grad()
            for i, batch in tqdm(
                enumerate(train_loader), desc="Batch", total=len(train_loader)
            ):
    ...
    def predict(self, x_test, progress=True):
        """
        ...
        test_encodings = self.tokenizer(
            test_texts, truncation=True, padding=True, max_length=self.tokenizer_max_len
        )
    ...
    def evaluate(self, x_val, y_val):
        ...
        val_labels = (
            y_val.to_list()
        )  # list of validation labels (ints from 0 to num_labels-1)
        val_encodings = self.tokenizer(
            val_texts, truncation=True, padding=True, max_length=self.tokenizer_max_len
        )
