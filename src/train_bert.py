...
import time
from utils.constants import MODEL_CV_RESULT_PATH, TARGET_INV_DICT, TARGET_INV_DICT_FASHION
...
def main(args):
    ...
    if args.cv:
        ...
        disp.plot(ax=ax)
        plt.savefig(f"{args.out_folder}/{experiment_name}/confusion_matrix_validation.png")
        ...
        predicted_probs = [sorted(zip(range(args.num_labels), probas), key=lambda x: x[1], reverse=True) for probas in probas_list]
        ...
        wrong_predictions = wrong_predictions[wrong_predictions[y_val_column] != wrong_predictions["predicted"]]

        # Save wrong predictions to a CSV file
        wrong_predictions.to_csv(f"{args.out_folder}/{experiment_name}/validation_wrongs.csv", index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default="dbmdz/bert-base-turkish-128k-uncased")
