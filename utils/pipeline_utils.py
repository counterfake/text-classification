...
def run_cv(
    ...
    for fold_id in tqdm(range(0, input_df[fold_col].max() + 1), desc="Training.. Fold"):
        ...
        X_train = preprocess_text(
            X_train, language="english", prevent_bias=prevent_bias
        )
        ...
        if prevent_bias == 2:
            X_train, y_train = add_external_positive_data(
                x_series=X_train, y_series=y_train
            )
    ...
    print(
        classification_report(
            input_df["target"], input_df["pred"], output_dict=False, digits=4
        )
    )
    ...
    input_df.to_csv(f"{MODEL_CV_RESULT_PATH}/{experiment_name}_OOF.csv", index=False)

    if add_to_zoo:
        zoo_member_dict = {
            fold_col: {
                experiment_name: classification_report(
                    input_df["target"], input_df["pred"], output_dict=True
                )
            }
        }
